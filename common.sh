...
func_generate() {
	...
	if [ "$BUILD_RESCUE" != "y" ]; then
		# depart image
		dd if=$tmpdir/$DISK of=$tmpdir/mbr-${DISK} bs=512 count=1 conv=fsync status=none
		dd if=$tmpdir/$DISK of=$tmpdir/ext4-${DISK} bs=2M skip=1 conv=fsync status=none
	fi
...
func_release() {
	...
	if [ "$BUILD_RESCUE" = "y" ]; then
		...
		echo "add ${os} imgs to rescue rootfs"
		mv -f $tmpdir/mbr-${DISK} $rootfs_mount_point/root
		mv -f $tmpdir/ext4-${DISK} $rootfs_mount_point/root
